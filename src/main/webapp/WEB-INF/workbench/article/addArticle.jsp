...
<script>
    ...
    $.get("/article/queryCategory",function (data) {
        //List<Category>
        for(var i = 0; i < data.length; i++){
    ...
    $('#categories').change(function () {
        $.get("/article/queryTags",{'cid':$(this).val()},function (data) {
            //List<Tag>
            //清空内容
            $('#tags').html("");
            for(var i = 0; i < data.length; i++){
    ...
    function punish() {
       ...
       $.post("/article/saveOrUpdate",{
            ...
            'logo' : $('#logo').val(),
            'content' : $('#content').val(),
            'isOpen' : $('input[type=radio]:checked').val()//需要进行判断勾中的传到后台去
        },function (data) {
            //返回resultVo
            if(data.ok){
              alert("发布成功")
                ...
                //跳转到文章列表页面
                location.href = "/toView/workbench/article/index";
            }
        },'json');
    }
</script>
